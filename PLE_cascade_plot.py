...
zc_a = [i+1 for i in zc_b]
print(zc_b)

cut_x = [PLE_x[0:zc_a[0]]] + [PLE_x[i:j] for i,j in zip(zc_a, zc_a[1:] + zc_a[:1])]
cut_y = [PLE_y[0:zc_a[0]]] + [PLE_y[i:j] for i,j in zip(zc_a, zc_a[1:] + zc_a[:1])]

print(cut_x)

for i, j in zip(cut_x, cut_y):
    plt.plot(i,j)
plt.show()
