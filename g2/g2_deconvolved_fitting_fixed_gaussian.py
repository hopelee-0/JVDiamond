...
def gaussian(t):
    w = 300/res
    return(1/w/np.sqrt(2*3.14159)*np.exp(-t**2/2/w**2))

# Define convolved signal
# def signal(t, tau, b, amp, g_back):
     # returns array with single item, so convert to scalar
    # return(g2(t, tau, b)*gaussian(t, amp, g_back))
    # return(np.convolve(g2(t, tau, b), gaussian(t, amp, g_back)))

def signal(g2_list, gaussian_list):
    return(np.convolve(g2_list, gaussian_list, 'same'))
